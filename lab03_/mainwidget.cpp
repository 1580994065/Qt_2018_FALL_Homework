...
extern enum SourceType sourcetype;
    ...
    ui(new Ui::mainWidget)
{
    ...
    ui->status->setIconSize(q.size());
    ui->status->setIcon(QIcon(q));
    ui->status->setStyleSheet("border:none;");
    ...
    connect(worker,&dataWorker::statusFlash,this,&mainWidget::setStatusIcon);
...
void mainWidget::setStatusIcon(QString file)
{
    QPainter paint;
    QPixmap q(100,100);
    q.load(file);
    paint.drawPixmap(0,0,100,100,q);
    ui->status->setIcon(QIcon(q));
    ui->status->setVisible(true);//显示图片
}
...
void mainWidget::on_status_clicked()
{
    switch(sourcetype)
    {
    case file:QDesktopServices::openUrl(QString(QCoreApplication::applicationDirPath())+"/data");  break;//打开文件夹
    case net:QDesktopServices::openUrl(QUrl(worker->geturl()));break;//打开网页
    default:break;
    }
}
